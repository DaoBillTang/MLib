...
import com.dtb.core.ConstVal.NO_GETTER
import com.dtb.core.Internals
import kotlin.DeprecationLevel.ERROR


class AppcompatAlertBuilder(override val ctx: Context) : AlertBuilder<AlertDialog> {
    private val builder = AlertDialog.Builder(ctx)

    override var title: CharSequence
        @Deprecated(NO_GETTER, level = ERROR) get() = Internals.noGetter()
        set(value) {
...
fun createAlertDialog(ctx: Context, init: AlertBuilder<*>.() -> Unit): AlertDialog {
    return AppcompatAlertBuilder(ctx).apply(init).build()
}

fun showAlertDialog(ctx: Context, init: AlertBuilder<*>.() -> Unit): AlertDialog {
    return AppcompatAlertBuilder(ctx).apply(init).show()
}